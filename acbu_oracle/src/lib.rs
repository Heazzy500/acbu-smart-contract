...
impl OracleContract {
    ...
    /// `sources` are the raw per-feed rates: every submission must contain at
    /// least `max(min_signatures, MIN_ORACLE_SOURCE_FEEDS)` entries (AC-014),
    /// after which the median is taken and feeds deviating beyond
    /// [`OUTLIER_THRESHOLD_BPS`] are discarded as outliers (emitting
    /// `OutlierDetectionEvent`). Updates are rate-limited to the configured
    /// update interval unless the new rate deviates beyond the per-currency
    /// emergency threshold **and** at least `min_signatures` validators have all
    /// independently cast emergency votes via [`Self::cast_emergency_vote`]
    /// (N-of-M consensus). A single validator can no longer unilaterally bypass
    /// the time-lock. Rejects timestamps older than the stored rate. The
